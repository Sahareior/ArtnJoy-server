...
const stripe = require("stripe")(
  "sk_test_51NEo1rESJveeRr2D7C6p7latFyClahoQ1dj9pw2qLzm7D088AbwYkUkY4l3LIT9NPStLZseQMGOlUCqWxXJvRGwf00BQatOtqb"
);
  ...
  try {
    ...
    app.patch("/class/:id", async (req, res) => {
      ...
      if (data.data.length > 8) {
        // Check the length of data.data instead of data.length
        updateData = {
          $set: {
            feedback: data.data,
          },
    ...
    app.post("/students", async (req, res) => {
      try {
        ...
        const existingData = await studentCollections.findOne({
          instructor: data.instructor,
        });
        if (existingData) {
